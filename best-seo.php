<?php
/*
Plugin Name: Expand SEO
Plugin URI: https://melotheme.com
...
*/
add_action('save_post', 'bestseo_save_moz_keyword',10,2);
add_filter('manage_posts_columns', 'bestseo_add_seo_columns', 10, 2);
add_action('manage_posts_custom_column', 'bestseo_display_seo_columns', 10, 2);
add_filter('manage_pages_columns', 'bestseo_add_seo_columns_pages', 10, 2);
add_action('manage_pages_custom_column', 'bestseo_display_seo_columns', 10, 2);
add_action('admin_head', 'moz_customize_css');
add_action('admin_menu', 'bestseo_post_options_box');
$moz = get_option('seo_settings');
if(isset($moz['no_cat_base'])){
	add_filter('category_rewrite_rules', 'bestseo_category_base_rewrite_rules');
	add_filter('query_vars', 'bestseo_category_base_query_vars');
	add_filter('request', 'bestseo_category_base_request');
	add_filter('category_link', 'bestseo_category_base',1000,2);
	add_action('created_category','bestseo_category_base_refresh_rules');
	add_action('edited_category','bestseo_category_base_refresh_rules');
	add_action('delete_category','bestseo_category_base_refresh_rules');
}
	
function bestseo_post_options_box() {
	if ( function_exists('add_meta_box') ) { 
		add_meta_box('rock-seo', __('Best SEO'), 'moz_rock_seo', 'post', 'side', 'high');
		add_meta_box('rock-seo', __('Best SEO'), 'moz_rock_seo', 'page', 'side', 'high'); 
		}		
	$moz_dir = plugins_url('/img', __FILE__);
	add_options_page( 'Best Settings', 'Best Settings', 'manage_options', 'best-seo-admin.php', 'seo_settings_admin', $moz_dir.'/favicon.png', 'top');
	register_setting( 'seo_settings_options', 'seo_settings', 'seo_settings_validate' );
}

/*category base*/
function bestseo_category_base_refresh_rules() {wp_cache_flush();global $wp_rewrite;$ce4_permalinks = get_option('permalink_structure');$wp_rewrite->set_permalink_structure($ce4_permalinks);$wp_rewrite->flush_rules();}
function bestseo_category_base_deactivate() {remove_filter('category_rewrite_rules', 'bestseo_category_base_rewrite_rules');bestseo_category_base_refresh_rules();}
function bestseo_category_base($catlink, $category_id) {$category = &get_category( $category_id );if ( is_wp_error( $category ) )return $category;$category_nicename = $category->slug;if ( $category->parent == $category_id )$category->parent = 0;elseif ($category->parent != 0 )$category_nicename = get_category_parents( $category->parent, false, '/', true ) . $category_nicename;$catlink = trailingslashit(get_option( 'home' )) . user_trailingslashit( $category_nicename, 'category' );return $catlink;}
function bestseo_category_base_rewrite_rules($category_rewrite) {$category_rewrite=array();$categories=get_categories(array('hide_empty'=>false));foreach($categories as $category) {$category_nicename = $category->slug;if ( $category->parent == $category->cat_ID )$category->parent = 0;elseif ($category->parent != 0 )$category_nicename = get_category_parents( $category->parent, false, '/', true ) . $category_nicename;$category_rewrite['('.$category_nicename.')/(?:feed/)?(feed|rdf|rss|rss2|atom)/?$'] = 'index.php?category_name=$matches[1]&feed=$matches[2]';$category_rewrite['('.$category_nicename.')/page/?([0-9]{1,})/?$'] = 'index.php?category_name=$matches[1]&paged=$matches[2]';$category_rewrite['('.$category_nicename.')/?$'] = 'index.php?category_name=$matches[1]';}global $wp_rewrite;$old_base = $wp_rewrite->get_category_permastruct();$old_base = str_replace( '%category%', '(.+)', $old_base );$old_base = trim($old_base, '/');$category_rewrite[$old_base.'$'] = 'index.php?category_redirect=$matches[1]';return $category_rewrite;}
function bestseo_category_base_query_vars($public_query_vars) {$public_query_vars[] = 'category_redirect';return $public_query_vars;}
function bestseo_category_base_request($query_vars) {if(isset($query_vars['category_redirect'])) {$catlink = trailingslashit(get_option( 'home' )) . user_trailingslashit( $query_vars['category_redirect'], 'category' );status_header(301);header("Location: $catlink");exit();}return $query_vars;}
function seo_settings_validate($input) {if($input['no_cat_base']==0) {bestseo_category_base_deactivate();}global $wp_rewrite;$wp_rewrite->flush_rules();return $input;}
function seo_settings_admin(){global $wp_rewrite;$wp_rewrite->flush_rules();include_once dirname(__FILE__) . '/best-seo-admin.php';}
...
function moz_filter_plugin_meta($links, $file) {if ( $file == MOZ_BASENAME ) {array_unshift($links,sprintf( '<a href="options-general.php?page=best-seo-admin.php">Settings</a>', MOZ_FILENAME, __('Settings') ));}return $links;}global $wp_version;if ( version_compare( $wp_version, '2.8alpha', '>' ) )add_filter( 'plugin_row_meta', 'moz_filter_plugin_meta', 10, 2 );add_filter( 'plugin_action_links', 'moz_filter_plugin_meta', 10, 2 );
...
function bestseo_save_moz_keyword($postID, $post) {
...
#moz_get_related_keywords {margin:-18px -3px 0 0;}
	...
	<?php	}
register_activation_hook(__FILE__, 'bestseo_post_options_box');
?>